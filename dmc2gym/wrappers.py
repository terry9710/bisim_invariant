...
from dmc2gym import natural_imgsource_invariant
...
class DMCWrapper(core.Env):
    def __init__(
        ...
        if img_source is not None:
            ...
            if img_source == "color":
                self._bg_source = natural_imgsource_invariant.RandomColorSource(shape2d)
                ...
                files = [
                    os.path.join(resource_files, f) for f in os.listdir(resource_files)
                    if os.path.isfile(os.path.join(resource_files, f))]
                if img_source == "images":
                    ...
                    self._bg_source = natural_imgsource_invariant.RandomVideoSource(shape2d, files, grayscale=False)
                else:
    ...
    def reset(self):
        ...
        obs = self._get_obs(time_step)
        env_index = self._bg_source.get_envindex()
        return obs, env_index
