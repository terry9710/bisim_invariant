...
#原代码的transition reward loss中reward的部分
    def update_reward_model(self, obs, action, next_obs, reward, env_index, L, step ):
        ...
        pre_reward = self.reward_decoder(pred_next_latent)
        pre_reward_env = self.reward_env_decoder(torch.cat([pred_next_latent, env_index], dim=1))

        reward_loss = F.mse_loss(pre_reward, reward)
        reward_env_loss = F.mse_loss(pre_reward_env, reward)
