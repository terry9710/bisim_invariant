...
import utils_invariant as utils
from logger import Logger
from video import VideoRecorder

from agent.baseline_agent import BaselineAgent
from agent.invariant_agent import BisimAgent
...
def parse_args():
    ...
    parser.add_argument('--resource_files', default='./videos/train',type=str)
    parser.add_argument('--eval_resource_files', default='./videos/test', type=str)
    parser.add_argument('--img_source', default='color', type=str, choices=['color', 'noise', 'images', 'video', 'none'])
    ...
    parser.add_argument('--num_train_steps', default=1000000, type=int) #1000000
...
################
    ...
    parser.add_argument('--work_dir', default='./waleker_run', type=str)
    parser.add_argument('--save_tb', default='store_true', action='store_true')
    parser.add_argument('--save_model', default='store_true', action='store_true')
    parser.add_argument('--save_buffer', default=None, action='store_true')
    parser.add_argument('--save_video', default='store_true', action='store_true')
...
def main():
    ...
    if args.encoder_type.startswith('pixel'):
        env = utils.FrameStack_index(env, k=args.frame_stack)
        eval_env = utils.FrameStack_index(eval_env, k=args.frame_stack)

    utils.make_dir(args.work_dir)
    video_dir = utils.make_dir(os.path.join(args.work_dir, 'video_log'))
    model_dir = utils.make_dir(os.path.join(args.work_dir, 'model_log'))
    ...
    for step in range(args.num_train_steps):
        ...
        episode_reward += reward
        replay_buffer.add(obs, action, curr_reward, reward, next_obs, done_bool, env_index)
