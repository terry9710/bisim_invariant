...
import random
from dmc2gym.natural_imgsource_invariant import RandomVideoSource
import glob
...
################
        self.env_index = np.empty((capacity, 1), dtype=np.float32)
##################
    ...
    def add(self, obs, action, curr_reward, reward, next_obs, done, env_index):
...
###################
        np.copyto(self.env_index[self.idx], env_index)
###################
...
##################
        env_index = torch.as_tensor(self.env_index[idxs], device=self.device)
######################
        if k:
            return obses, actions, rewards, next_obses, not_dones, torch.as_tensor(self.k_obses[idxs], device=self.device)
        return obses, actions, curr_rewards, rewards, next_obses, not_dones, env_index
...
#######################
            self.env_index[self.last_save:self.idx]
##########################
        ]
...
# class FrameStack_index(gym.Wrapper):
#     def __init__(self, env, k, bg_path, height=256, width=256, ):
#         gym.Wrapper.__init__(self, env)
#         self._k = k
#         self._frames = deque([], maxlen=k)
#         shp = env.observation_space.shape
...
#         self._bg_source = RandomVideoSource((height, width), files, grayscale=False)
#
# #################
#     def reset(self):
#         obs = self.env.reset()
...
#         # bg, env_index = self._bg_source.get_image()
#         # frame[mask] = bg[mask]
# #################
...
#         self._frames.append(obs)
#
#         return self._get_obs(), reward, done, info, env_index
#
#     def _get_obs(self):
#         assert len(self._frames) == self._k
#         return np.concatenate(list(self._frames), axis=0)
...
class FrameStack_index(gym.Wrapper):
    ...
    def reset(self):
        obs, env_index = self.env.reset()
        for _ in range(self._k):
            self._frames.append(obs)
        return self._get_obs(), env_index
